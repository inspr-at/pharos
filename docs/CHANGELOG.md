...
- Refuse to confirm an Aeon launch when the fetched handoff reports `authority_open` false or `superseded_by` is set, and journal that as `handoff_authority_not_open`, which maps to `policy_refused`, while the handoff is still inside `expires_at`. A handoff already past `expires_at` is journaled as `admission_expired` instead. When those fields are absent, confirmation still infers an open authority from the handoff state, an empty result, and the journaled admission (PHAROS-321).
- Remember the first handoff read from an Aeon origin that includes `authority_open`, and keep that fact in the delivery journal. A later handoff read that omits the field journals `authority_signal_missing` (`policy_refused`) and does not confirm the host job. An origin that never sends the field keeps the inferred check (PHAROS-321).
- Bind each Aeon delivery journal to one origin, and remember `authority_open` separately for every origin recorded in that journal. A journal with no origin record still starts and records the configured origin on the first handoff read. Startup refuses when the configured origin differs from an origin the journal already records; a new origin needs its own journal path. An origin that has sent `authority_open` still requires it after another origin is recorded, and a later omission journals `authority_signal_missing` (`policy_refused`) without confirming the host job (PHAROS-321).
...
- Enforce HTTPS for OIDC provider requests and verify scoped private-CA trust with real HTTPS acceptance, rejection and login-flow tests (PHAROS-272).
...
- Render calendar v2 versions in the sidebar pill and the release sheet with the doctrine display weights: per-segment opacity and the deep-accent tint on the date, read at build time from `contracts/inspr-calendar-version-display/calendar-version-display.json`, an in-repo copy pinned byte-for-byte to `doctrine/lib/calendar-version-display.json` (inspr-modules 0.9.0) by `scripts/check-calendar-version-display.sh`. The element's text content and `data-version` stay the canonical label; v1 and legacy versions render plain (PHAROS-261, INSPR-400).
...
- Adopt the republished Paimos external-stage v2 contract (schema, owner fixture with the calendar v2 deployment and verification pair, re-certified manifest) so Paimos delivery handoffs can carry v2 identities.
